...
public class Exercicio04 {
	...
	static Conta[] arrayConta = new Conta[10];
	...
	public static void main(String[] args) {
		...
		System.out.println(
				"                                                     FÉ NO PAI QUE O CODIGO SAI                    ");

		// arrays
		System.out.println("    Conta Poupança");

		adicionarArrayConta(contaPoupanca);
		adicionarArrayConta(contaPoupanca1);
		adicionarArrayConta(contaPoupanca2);
		adicionarArrayConta(contaPoupanca3);
		adicionarArrayConta(contaPoupanca4);
		adicionarArrayConta(contaPoupanca5);

		System.out.println("    Conta Corrente");
		adicionarArrayConta(contaCorrente);
		adicionarArrayConta(contaCorrente1);
		adicionarArrayConta(contaCorrente2);
		adicionarArrayConta(contaCorrente3);
		adicionarArrayConta(contaCorrente4);
		adicionarArrayConta(contaCorrente5);

		System.out.println("    Conta Empresarial");
		adicionarArrayConta(contaEmpresarial);
		adicionarArrayConta(contaEmpresarial1);
		adicionarArrayConta(contaEmpresarial2);
		adicionarArrayConta(contaEmpresarial3);
		adicionarArrayConta(contaEmpresarial4);
		adicionarArrayConta(contaEmpresarial5);

		primeiroElementoArrayConta();
		ultimoElementoArrayConta();
		tamanhoElementoDoArrayConta();

	}

	private static void adicionarArrayConta(Conta conta) {
	...
	private static void primeiroElementoArrayConta() {
		if (arrayConta[0] != null) {
			System.out.println(arrayConta[0].getClientes().getNomeCliente());
		} else {
	...
	private static void tamanhoElementoDoArrayConta() {
		int contador = 0;
		...
		System.out.println("Seu array possui " + contador + " do total de " + arrayConta.length);

	}

	private static void ultimoElementoArrayConta() {
		Conta conta = null;
		if (arrayConta[0] != null) {
			for (int i = 0; i < arrayConta.length; i++) {
				if (arrayConta[i] != null) {
